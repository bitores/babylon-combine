...
var mergeBabylonFile = function(arrSrc, outFile, useBinary){
    ...
    if(useBinary) {
        if(outFile.indexOf("binary.babylon")===-1) {
            outFile = path.dirname(outFile) +'/'+ path.basename(outFile) + ".binary.babylon";
        }
    } else {
        if(outFile.indexOf("babylon")===-1) {
            outFile = path.dirname(outFile) +'/'+ path.basename(outFile) + ".babylon";
        }
    }
