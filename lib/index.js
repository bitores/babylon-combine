...
module.exports = {
	...
	createModule: function(arv){
		...
		var filesDir = path.join( process.cwd(), arv[0]),
            ...
            if(/\.binary\.babylon$/.test(path.basename(outFile)) === true) {
                console.log('useBinary = true', outFile, path.basename(outFile))
                useBinary = true;
            } else if(/\.babylon$/.test(path.basename(outFile)) === true) {
                console.log('useBinary = false', outFile, path.basename(outFile))
                useBinary = false;
            }else{
                outFile = './outdir/out';
            }
		...
		if(useBinary === true ) {
            ...
            console.log("aa..........")
			mergeBabylonFile(binaryBabylonFiles, path.dirname(outFile) +'/'+ path.basename(outFile) + ".babylon",true);
			mergeBabylonFile(babylonFiles, path.dirname(outFile) +'/'+ path.basename(outFile) + ".binary.babylon",false);
		}
